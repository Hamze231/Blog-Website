...
from firstapp.forms import CommentForm, NewUserForm, SubscribeForm
from firstapp.models import Comments, Post, Profile, Tag, WebsiteMeta
...
def all_bookmarked_posts(request):
    ...
    return render(request,'firstapp/all_bookmarked_posts.html',context )

def all_posts(request):
    all_posts = Post.objects.all()
    context = {"all_posts":all_posts}
    return render(request,'firstapp/all_posts.html',context )

def all_liked_posts(request):
    all_liked_posts = Post.objects.filter(likes=request.user)
    context = {"all_liked_posts":all_liked_posts}
    return render(request,'firstapp/all_liked_posts.html',context )
